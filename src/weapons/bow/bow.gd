...
export (int) var multishot_count = 200
...
func attack(degrees, angle, damage, knockback_power):
	one_shoot(degrees, angle, damage, knockback_power)

	if multishot_passive.is_active:
		attack_counter += 1
		if multishot_passive.count_proc <= attack_counter:
			cast_skill_1(degrees, angle, damage, knockback_power)
			attack_counter = 0
...
func cast_skill_1(degrees, angle, damage, knockback_power):
	var casted = 0
	var add_angle = 0
	var add_degrees = 0
	...
	while casted < multishot_count:
		if is_right_side:
			one_shoot(degrees + add_degrees, angle + add_angle, damage, knockback_power)
		else:
			one_shoot(degrees - add_degrees, angle - add_angle, damage, knockback_power)
		add_angle += 0.02
		add_degrees += 2
...
func one_shoot(degrees, angle, damage, knockback_power):
